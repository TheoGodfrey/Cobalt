...
// --- FIX 3.3: Define separate icons for online and offline states ---
// Custom drone icon (ONLINE)
const droneIconOnline = L.divIcon({
    html: `
...
// Custom drone icon (OFFLINE)
const droneIconOffline = L.divIcon({
    html: `
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-8 h-8 text-gray-500 opacity-70">
            <path d="M12.001 2.25c.578 0 1.122.164 1.586.444l4.359 2.5c.465.268.86.621.112.985l-4.359 2.5c-.465.268-.86.621-1.12.985l-4.36-2.5c-.464-.268-.859-.621-1.119-.985l4.36-2.5c.464-.268.859-.621 1.119-.985zM12.001 7.119c.578 0 1.122.164 1.586.444l4.359 2.5c.465.268.86.621.112.985l-4.359 2.5c-.465.268-.86.621-1.12.985l-4.36-2.5c-.464-.268-.859-.621-1.119-.985l4.36-2.5c.464-.268.859-.621 1.119-.985zM12.001 11.984c.578 0 1.122.164 1.586.444l4.359 2.5c.465.268.86.621.112.985l-4.359 2.5c-.465.268-.86.621-1.12.985l-4.36-2.5c-.464-.268-.859-.621-1.119-.985l4.36-2.5c.464-.268.859-.621 1.119-.985zM21.75 12.14c.118-.458-.112-.94-.56-1.115l-4.36-2.5a1.875 1.875 0 00-1.688 0l-4.36 2.5c-.448.175-.678.657-.56 1.115l.397 1.548c.118.458.55.764 1.008.688l3.201-.54c.458-.076.89.23 1.008.688l.397 1.548c.118.458.55.764 1.008.688l3.201-.54c.458-.076.89.23 1.008.688l.397 1.548z" />
        </svg>`,
    className: 'bg-transparent border-0',
    iconSize: [32, 32],
 ...
 * @param {string} status - The current state of the drone (e.g., "OFFLINE", "SEARCHING")
 */
// --- FIX 3.3: Modified function signature to accept 'status' ---
export function updateDroneMarker(droneId, lat, lon, heading = 0, status = 'ONLINE') {
// --- End of FIX 3.3 ---
    ...
    // --- FIX 3.3: Determine which icon and style to use ---
    const isOffline = (status === 'OFFLINE');
    const iconToUse = isOffline ? droneIconOffline : droneIconOnline;
    const tooltipClass = isOffline ? 
        'bg-gray-700 text-gray-400 border-0 rounded-md p-2 shadow-lg opacity-80' : 
        'bg-gray-800 text-white border-0 rounded-md p-2 shadow-lg';
    // --- End of FIX 3.3 ---
    ...
    if (droneMarkers[droneId]) {
        ...
        // --- FIX 3.3: Update icon based on status ---
        droneMarkers[droneId].setIcon(iconToUse);
        // Re-bind tooltip to update its class
        droneMarkers[droneId].unbindTooltip().bindTooltip(tooltipContent, {
            permanent: true,
            direction: 'top',
            offset: [0, -15],
        ...
        droneMarkers[droneId] = L.marker(latLng, {
            // --- FIX 3.3: Use the correct icon on creation ---
            icon: iconToUse,
            // --- End of FIX 3.3 ---
        ...
        droneMarkers[droneId].bindTooltip(tooltipContent, {
            ...
            direction: 'top',
            offset: [0, -15],
            // --- FIX 3.3: Use the correct class on creation ---
            className: tooltipClass
            // --- End of FIX 3.3 ---
        });
    }

    // Pan map to the latest drone only if it's not offline
    if (!isOffline) {
        map.panTo(latLng);
    }
}
